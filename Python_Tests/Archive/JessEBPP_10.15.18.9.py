...
cursor.execute(
    "CREATE TABLE Test_11 (Ccode_l VARCHAR(255), spname_l VARCHAR(255), Mdata_l VARCHAR(255), running_num VARCHAR(255))")
...
for Ccode_l in WD:
    for spname_l in WD[Ccode_l]:
        for Mdata_l in WD[Ccode_l][spname_l]:
            ...
            sql = "INSERT INTO Test_11 (Ccode_l,spname_l,Mdata_l,running_num) VALUES (%s, %s, %s, %s)"
            val = (Ccode_l, spname, Mdata_l, running_num)
            print("inserted" + str(val) + "into" + "Test_11")
